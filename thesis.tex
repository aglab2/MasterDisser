...
Для представления полученных данных на диске необходимо хранить карту блоков, которые можно отдать на чтение. В данном разделе будут рассмотрены структуры данных, позволяющие работать с такой картой.

Рассмотрим действия над двумя картами $A$ и $B$. \emph{Объединение} ($A \cup B$) двух карт есть новая карта, которая содержит блоки обеих карт. \emph{Разностью} ($A \setminus B$) двух карт будем называть функцию, которая возвращает карту, которая содержит все блоки карты $A$ за исключением блоков карты $B$. Действием \emph{пересечение} является действие вида $A \setminus (A \setminus B)$.

Для имплементация класса $BlockRegions$ будем рассматривать действия, которые будут изменять сам объект, но не возвращать новый экземпляр. Интерфейс представлен в листинге \ref{fig:blockregions}.
...
Блоки представлены в виде полуотрезков вида $[start, end),\ start<end,$ $length=start-end$, указывающих на часть файла, которая была загружена. Такой полуотрезок будем называть \emph{регион} ($BlockRegion$). 

$BlockRegions$ дает интерфейс для работы с картой блоков регионов. Действия \emph{объединение} и \emph{разность} представлены в виде функций $add$ и \\ $checkAndRemove$. $checkAndRemove$ также возвращает информацию о существовании пересечения между регионом и картой.

$BlockRegionsIterator$ предоставляет интерфейс для получения списка регионов, хранящихся в $BlockRegions$. $Iterator$ можно использовать для расширения методов $add$ и $checkAndRemove$ до функций работы с другой картой регионов $...(BlockRegions*)$, что даст реализацию действий \emph{объединения} и \emph{пересечения} над двумя картами (смотреть листинг \ref{fig:convenienceregions}).
...
Нвзовем состояние объекта $BlockRegions\_Disjoint$ валидным, если выполнены 4 условия:
\begin{enumerate}
\item Нет пересечений регионов,
\item Регионы в списке отсортированы по возрастанию,
\item Отсутствуют регионы нулевой длины,
\item Конец региона не должен совпадать с началом следующего региона.
\end{enumerate}
...
Метод $merge$ является защищенным методом и помогает восстановить список из невалидного состояния при нарушении свойства 1 или 4: наличие пересекающихся регионов как отрезков, при этом будем требовать выполнения свойств 2 и 3. Без ограничения общности можно считать, что в списке есть единственный регион, склеивание которого со следующими регионами восстановит валидность списка. Назовем такой регион $reason$(причина). Аргумент функции $merge$ есть регион $reason$, после которого происходит нарушение свойства 1 или 4, то есть регионы $reason$ и $reason \rightarrow next$ как отрезки пересекаются. Псевдокод метода $merge$ приведен в листинге \ref{fig:regionsmerge}.

\begin{lstlisting}[style=CStyle, caption={Псевдокод функции $merge$}, label=fig:regionsmerge]
void merge(BlockRegion* reason)
{
  if (reason == sentinel) return;

  // Find if next is intersecting
  while (reason->next != NULL && END(reason) >= START(reason->next))
  {
    // Merge the region and adjust reason
    merged = reason->next;
    if (END(reason) < END(reason->next))
      reason->length = END(reason->next) - START(reason);

    reason->next = merged->next;
    delete merged;
  }
}
\end{lstlisting}

Метод $merge$ проверяет, что следующий регион $reason \rightarrow next$ пересекается с $next$ и изменяет размер $reason$, если требуется. Так как выполнено регионы отсортированы (свойство 2), то $while\ loop$ достаточно для проверки всех необходимых регионов. Заметим, что не всегда необходимо изменять длину региона $reason$: если $reason \rightarrow next$ находится полностью внутри $reason$, то изменение длины не требует.
...
При существующей связи $user \rightarrow kext$ организация обратной связи $kext \rightarrow user$ возможно многими способами. В данной работе будет рассмотрена очередь сообщений $IOSharedDataQueue$ как транспорт $kext \rightarrow user$.

$Kext$ создает экземпляр очереди с указанием размера, например, во время присоединения клиента. Клиент после инициализации создает порт для передачи данных через $IODataQueueAllocateNotificationPort$, вызывает функцию $IOConnectMapMemory$ и получает указатель на разделяемую память $IODataQueueMemory$ для работы с очередью. Далее клиент ждет событий о добавлении новых данных при помощи $DataAvailable$ и получает их при поступлении через $Dequeue$. Пример кода клиента представлен в листинге \ref{fig:userdataqueue}.
...
Со стороны $Kernel$ $kext$ должен зарегистрировать порт, переопределяя метод $IOUserClient::registerNotificationPort$ и устанавливая его в созданную очередь при помощи метод $IOSharedDataQueue::setNotificationPort$. Для отправки сообщений $kext$ должен использовать метод $IOSharedDataQueue::enqueue$, который \emph{асинхронно} добавляет новый элемент в очередь.

\subsection{Синхронный транспорт}
\label{sec:reqresp}
Для работы виртуальных файлов на операции $Read$ должна происходить \emph{синхронная} загрузка данных, в противном случае $Read$ может считать еще нескаченные данные. Сформулируем требования для транспорта, достаточные для организации виртуальных файлов.

\begin{enumerate}
\item Синхронность. $Kext$ на операции чтения должен ожидать пока сервис не загрузит корректные данные файла,
...
Прототипы функций представлены в листинге \ref{fig:syncprot}.
...
Клиент в ответ вызывает метод $kext$ для разблокирования синхронного сообщения, $kext$ разрешает чтение файла.

Опишем дополнительное методы управления $kext$. Для сохранения информации о регионах файла добавим функции для доступа к расширенным атрибутам или к \emph{per vnodе} хранилищу. $Kext$ также будет кэшировать информацию о статусе файла, обеспечивая доступ к кэшу через функции $addToCache$ и $removeFromCache$. Для изменения содержимого файла со стороны клиента введем функции блокировки операций чтения и записи $lockFileRead$ и $lockFileWrite$. Для очистки кэша после отгрузки файла в удаленное хранилище, введем метод для удаления $UBC$ кэша $invalidateVnodeCache$.

\chapter{Реализация и производительность}
\label{sec:impl}
...
При авторизации чтения из файла на $VnodeOp$ коллбэке в $KauthFilter$ производится проверка является ли файл виртуальным. Если файл виртуальный, запрос отправляется в \emph{Filter Manager}, который в порядке приоритетов вызывает фильтры. \emph{VOP Hook Filter} проверяет перехвачена ли таблица $VOP$ и производит ее перехват. \emph{Process Exclusion Filter} проверяет может ли процесс производить чтение файла и может заблокировать доступ нежелательных процессов. На операции $pagein$ в случае необходимости производится итерация по списку записей карты виртуальной памяти и подменяется $vm\_object$. \emph{Client} при помощи методов $Block\ Region$ находит ту часть файла, которую необходимо скачать с удаленного хранилища. Если пересечение непусто, отправляется синхронный запрос $UserSpace$ клиенту и выполняется ожидание, пока в файл не будут записаны данные.
...
\conclusion
В результате исследования было написано расширение ядра macOS, осуществляющее управление содержимым виртуальных файлов с удаленным хранилищем. При помощи перехватов операций $vnode$ остается поддержка $POSIX\ API$, а файлы при скачивании остаются локально. Парсинг виртуальной памяти отфильтрованных процессов позволяет тонко контролировать доступ к невалидным страницам файла. Описанный синхронный протокол дает возможность гибкого соединения с $UserSpace$ клиентом, при сбоях возможно использование тайм-аута и более простой проверки работоспособности.

Предложенные методы используются в Acronis для продукта Acronis Sync \& Share для macOS.
